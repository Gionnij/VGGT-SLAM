...
from ..datasets.dataset_io import (
    hdf5_write_hierarchy_to_file,
    hdf5_read_hierarchy_from_file,
)
from ..jupyter_show_image import (
    adapt_img_data,
    imwrite,
    imread,
...
def default_instancer(
    ...
    return (
        anomaly_instances[mask_roi],
        anomaly_seg_pred[mask_roi],
        segmentation_filtered,
    )


def anomaly_instances_from_mask(
    segmentation: np.ndarray, label_pixel_gt: np.ndarray, thresh_instsize: int = 0
):
    anomaly_gt = np.zeros(label_pixel_gt.shape)
    anomaly_gt[label_pixel_gt == 1] = 1
    anomaly_pred = np.zeros(label_pixel_gt.shape)
...
def segment_metrics(
    ...
    for i in np.unique(anomaly_instances[anomaly_instances > 0]):
        ...
        adjustment = len(
            anomaly_seg_pred[
                np.logical_and(
                    ~np.isin(anomaly_instances, [0, i]),
                    np.isin(anomaly_seg_pred, seg_ind),
                )
            ]
        )

        adjusted_union = (
            np.sum(np.isin(anomaly_seg_pred, seg_ind))
            + np.sum(anomaly_instances == i)
            - intersection
            - adjustment
        )
    ...
    for i in np.unique(anomaly_seg_pred[anomaly_seg_pred > 0]):
        ...
        adjustment = len(
            anomaly_instances[
                np.logical_and(
                    ~np.isin(anomaly_seg_pred, [0, i]),
                    np.isin(anomaly_instances, seg_ind),
                )
            ]
        )
        adjusted_union = (
            np.sum(np.isin(anomaly_instances, seg_ind))
            + np.sum(anomaly_seg_pred == i)
            - intersection
            - adjustment
        )
...
class MetricSegment(EvaluationMetric):
    ...
    def vis_frame(
        self, fid, dset_name, method_name, mask_roi, anomaly_p, image=None, **_
    ):
        ...
        imwrite(
            DIR_OUTPUTS / f"vis_SegPred" / method_name / dset_name / f"{fid}.webp",
            demo_img,
        )
    ...
    def process_frame(
        ...
        **_,
        ...
        if self.cfg.get(default_instancer, True):
            anomaly_gt, anomaly_pred, mask = default_instancer(
                anomaly_p,
                label_pixel_gt,
                self.cfg.thresh_p,
            ...
            anomaly_gt, anomaly_pred = anomaly_instances_from_mask(
                anomaly_mask, label_pixel_gt, self.cfg.thresh_instsize
            )
    ...
    def aggregate(self, frame_results: list, method_name: str, dataset_name: str):
        ...
        prec_pred_mean = sum(np.sum(r.prec_pred) for r in frame_results) / sum(
            len(r.prec_pred) for r in frame_results
        )
        ag_results = {
            "tp_mean": 0.0,
            "fn_mean": 0.0,
    ...
    def persistence_path_data(self, method_name, dataset_name):
        return (
            ...
            / self.name
            / "data"
            / f"{self.name}Results_{method_name}_{dataset_name}.hdf5"
        )
