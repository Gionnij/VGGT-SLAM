...
from .pixel_classification_curves import (
...
def frame_metrics(anomaly_p: np.ndarray, label_pixel_gt: np.ndarray):
    """Compute AUROC, AP (AUPR for anomaly=positive), FPR@95 for one frame."""
    mask_roi = label_pixel_gt < 255
    y_true = label_pixel_gt[mask_roi].astype(bool)  # 1=anomaly, 0=road
    y_score = anomaly_p[mask_roi]

    if np.all(y_true == 0) or np.all(y_true == 1):
        # degenerate case: no anomalies or all anomalies → undefined metrics
        return dict(AUROC=np.nan, AP=np.nan, FPR95=np.nan)
    ...
    # FPR@95TPR
    mask = tpr >= 0.95
    fpr95 = np.min(fpr[mask]) if np.any(mask) else np.nan

    # Precision-recall (anomaly=positive)
...
def binary_confusion_matrix(
    ...
    if bin_strategy == "uniform":
        ...
        eps = np.maximum(
            vrange * 1e-2, 1e-2
        )  # make sure there is some separation between the thresholds
...
class MetricPixelClassification(EvaluationMetric):
    ...
    def vis_frame(
        ...
        **_,
        ...
        canvas = (
            image.copy() if image is not None else np.zeros((h, w, 3), dtype=np.uint8)
        )
        heatmap_color = adapt_img_data(anomaly_p)
        canvas[mask_roi] = canvas[mask_roi] // 2 + heatmap_color[mask_roi] // 2
        imwrite(
            DIR_OUTPUTS
            / f"vis_PixelClassification"
            / method_name
            / dset_name
            / f"{fid}_demo_anomalyP.webp",
            canvas,
        )
        ...
        imwrite(
            ...
            / method_name
            / dset_name
            / f"{fid}_demo_anomalyP_heat.webp",
            anomaly_heat,
        )
    ...
    def process_frame(
        ...
        **_,
        ...
        try:
            ...
            raise RuntimeError(
                f"No ground truth available for {fid}. Please check dataset path..."
            )
        ...
        if (
            ...
            self.vis_frame(
                ...
                mask_roi=mask_roi,
                anomaly_p=(anomaly_p - anomaly_p.min())
                / (anomaly_p.max() - anomaly_p.min()),
                label_pixel_gt=label_pixel_gt,
                **_,
            )

        # print('Vrange', np.min(predictions_in_roi), np.mean(predictions_in_roi), np.max(predictions_in_roi))
        metrics = frame_metrics(anomaly_p, label_pixel_gt)
    ...
    def persistence_path_data(self, method_name, dataset_name):
        return (
            ...
            / self.name
            / "data"
            / f"PixClassCurve_{method_name}_{dataset_name}.hdf5"
        )
    def save_frame_results(self, frame_results, method_name, dataset_name):
        """
        Save the aggregated results of the pixel classification metric.
        """
        file = (DIR_OUTPUTS
            / self.name
            / f"frame_results_{method_name}_{dataset_name}.pkl")
        file.parent.mkdir(parents=True, exist_ok=True)
        print(f"=============================================Saving frame results to {file}===================================================================S")
    ...
    def save(
        ...
        c_lowres.save(
            out_path.with_name(
                out_path.name.replace("PixClassCurve", "PixClassCurve-simplified")
            )
        )
        if frame_results is not None:
            self.save_frame_results(frame_results, method_name, dataset_name)

    def load(self, method_name: str, dataset_name: str, path_override: Path = None):
        out_path = path_override or self.persistence_path_data(
            method_name, dataset_name
        )
        out_path_simplified = out_path.with_name(
            out_path.name.replace("PixClassCurve", "PixClassCurve-simplified")
        )
        ...
        if out_path.is_file():
            ...
            raise FileNotFoundError(
                f"No saved curve at {out_path} or {out_path_simplified}"
            )
    ...
    def plot_many(
        ...
        cinfos = [
            reduce_curve_resolution(cinfo, num_pts=256) for cinfo in aggregated_results
        ]

        vis_res = plot_classification_curves(
            cinfos, method_names=method_names, plot_formats=plot_formats
        )
