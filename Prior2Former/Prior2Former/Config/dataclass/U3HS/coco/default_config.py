...
class DatamoduleArgsConfigCoco(NestedConfig):
    ...
    crop_size: List[int] = field(default_factory=lambda: [640, 480])
    base_size: List[int] = field(default_factory=lambda: [640, 480])
...
class TrainerConfig(NestedConfig):
    ...
    trainer_args: Dict[str, Union[str, bool, float]] = field(
        default_factory=lambda: {"strategy": "ddp_find_unused_parameters_true"}
    )
    max_epochs: int = 80
