...
@dataclass
class SwinBackboneConfig(NestedConfig):
    name: str = "SwinTransformer"
    embed_dim: int = 96
...
class ModelConfig(NestedConfig):
    ...
    SWIN: SwinBackboneConfig = field(default_factory=lambda: SwinBackboneConfig())
...
class LRSchedulerConfig(NestedConfig):
    type: str = "polynomial"
    args: Dict[str, Union[float, int]] = field(default_factory=lambda: {"power": 0.9,
        "total_iters": 50})
