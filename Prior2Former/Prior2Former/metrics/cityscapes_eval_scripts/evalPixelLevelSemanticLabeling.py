...
train_id_to_id={label.trainId: label.id for label in labels}
train_id_to_id.update({-100:6})
# C Support
...
class CityScapesEvaluator():
    ...
    def __init__(self,args):
        self.args=args
        self.confMatrix = generateMatrix(args)
        self.instStats = generateInstanceStats(args)
        self.perImageStats = {}
        self.nbPixels = 0


    def update(self,predictionImgList, groundTruthImgList):
        ...
        for i in range(len(predictionImgList)):
            ...
            self.nbPixels += evaluatePairNP(predictionImgFileName, groundTruthImgFileName, self.confMatrix, self.instStats, self.perImageStats, self.args,load=False)

            # sanity check
            if self.confMatrix.sum() != self.nbPixels:
                printError(
    ...
    def compute(self):
        ...
        # print confusion matrix
        if (not self.args.quiet):
            printConfMatrix(self.confMatrix, args)

        # Calculate IOU scores on class level from matrix
        classScoreList = {}
        for label in self.args.evalLabels:
            labelName = id2label[label].name
            classScoreList[labelName] = getIouScoreForLabel(label, self.confMatrix, args)

        # Calculate instance IOU scores on class level from matrix
        classInstScoreList = {}
        for label in self.args.evalLabels:
            labelName = id2label[label].name
            classInstScoreList[labelName] = getInstanceIouScoreForLabel(label, self.confMatrix, self.instStats, args)

        # Print IOU scores
        if (not self.args.quiet):
            print("")
            print("")
            printClassScores(classScoreList, classInstScoreList, self.args)
            iouAvgStr = getColorEntry(getScoreAverage(classScoreList, self.args), self.args) + "{avg:5.3f}".format(
                avg=getScoreAverage(classScoreList, self.args)) + self.args.nocol
            niouAvgStr = getColorEntry(getScoreAverage(classInstScoreList, self.args), self.args) + "{avg:5.3f}".format(
                avg=getScoreAverage(classInstScoreList, self.args)) + self.args.nocol
            print("--------------------------------")
        ...
        for category in category2labels.keys():
            categoryScoreList[category] = getIouScoreForCategory(category, self.confMatrix, self.args)

        # Calculate instance IOU scores on category level from matrix
        categoryInstScoreList = {}
        for category in category2labels.keys():
            categoryInstScoreList[category] = getInstanceIouScoreForCategory(category, self.confMatrix, self.instStats, self.args)

        # Print IOU scores
        if (not self.args.quiet):
            print("")
            printCategoryScores(categoryScoreList, categoryInstScoreList, self.args)
            iouAvgStr = getColorEntry(getScoreAverage(categoryScoreList, self.args), self.args) + "{avg:5.3f}".format(
                avg=getScoreAverage(categoryScoreList, self.args)) + self.args.nocol
            niouAvgStr = getColorEntry(getScoreAverage(categoryInstScoreList, self.args), self.args) + "{avg:5.3f}".format(
                avg=getScoreAverage(categoryInstScoreList, self.args)) + self.args.nocol
            print("--------------------------------")
            print("Score Average : " + iouAvgStr + "    " + niouAvgStr)
            print("--------------------------------")
            print("")

        allResultsDict = createResultDict(self.confMatrix, classScoreList, classInstScoreList, categoryScoreList,
                                          categoryInstScoreList, self.perImageStats, self.args)
        # write result file
        if self.args.JSONOutput:
            writeJSONFile(allResultsDict, self.args)
...
# Main evaluation method. Evaluates pairs of prediction and ground truth
# images which are passed as arguments.
def evaluatePairNP(predictionImgFileName, groundTruthImgFileName, confMatrix, instanceStats, perImageStats, args, load=True):
    # Loading all resources for evaluation.

    predictionNp = np.array(predictionImgFileName)
    groundTruthNp = np.array(groundTruthImgFileName)
    # load ground truth instances, if needed
    if args.evalInstLevelScore:
        groundTruthInstanceImgFileName = groundTruthImgFileName.replace("labelIds", "instanceIds")
        try:
    ...
    # Check for equal image sizes
    if (predictionNp.shape[0] != groundTruthNp.shape[0]):
        printError("Image widths of " + predictionImgFileName + " and " + groundTruthImgFileName + " are not equal.")
    if (predictionNp.shape[1] != groundTruthNp.shape[1]):
        printError("Image heights of " + predictionImgFileName + " and " + groundTruthImgFileName + " are not equal.")
    if (len(predictionNp.shape) != 2):
        printError("Predicted image has multiple channels.")
    ...
    nbPixels = imgWidth * imgHeight
    groundTruthNp=np.vectorize(train_id_to_id.get)(groundTruthNp)
    predictionNp = np.vectorize(train_id_to_id.get)(predictionNp)
    # Evaluate images
    if (CSUPPORT):
        # using cython
        confMatrix = addToConfusionMatrix.cEvaluatePair(predictionNp, groundTruthNp, confMatrix, args.evalLabels)
...
def main():
    ...
    # however the no-argument way is prefered
        ...
        if not groundTruthImgList:
            print("Cannot find any ground truth images to use for evaluation. Searched for: {}".format(
...
def get_cityscapes_evaluator(save_path=None):
    args.evalInstLevelScore=False
    args.exportFile=save_path
    if save_path is None:
        args.JSONOutput=False
    return CityScapesEvaluator(args)
