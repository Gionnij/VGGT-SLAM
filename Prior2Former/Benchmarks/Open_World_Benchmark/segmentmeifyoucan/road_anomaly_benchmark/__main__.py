...
#from Open_World_Benchmark.estimators import get_estimator
...
# except:
#     from Open_World_Benchmark.segmentmeifyoucan.road_anomaly_beanchmark.evaluation import Evaluation
from .evaluation import Evaluation
...
try:
    ...
    from Benchmarks.Open_World_Benchmark.segmentmeifyoucan.road_anomaly_benchmark.metrics import MetricRegistry

def get_out_size(dset):
    out_sizes = {
        "ObstacleTrack-validation": (1080, 1920),
        "AnomalyTrack-validation": (720, 1280),
        "LostAndFound-testNoKnown": (1024, 2048),
        "FishyAnomaly_val_v1": (1024, 2048),
    }
    return out_sizes[dset]
...
@click.option("--key", type=str, default="anomaly_p")
...
def metric(
    ...
    for dset in dataset_names:
        for method in method_names:
            ...
            #     for arg in names[1:]:
            #         assert (
            #             "=" in arg and len(arg.split("=")) == 2
            #         ), f"Invalid argument {arg}"
            #     kwargs = {
            #         arg.split("=")[0]: float(arg.split("=")[1]) for arg in names[1:]
            #     }
            ...
            for method in method_names:
                try:
                    ...
                    for field, val in metric.extracts_fields_for_table(ag).items():
                        ...
                        col = columns.setdefault(col_name, Series(dtype=np.float64))
                        col[method] = val
...
def comparison(
    ...
    # Load metrics
    table = load_scores(
        method_names, metric_names, dataset_names, load_leaderboard=leaderboard
    )
    ...
    if names is not None:
        ...
        table = table.rename(
            columns=col_renames, index=methods_renames, errors="ignore"
        )
