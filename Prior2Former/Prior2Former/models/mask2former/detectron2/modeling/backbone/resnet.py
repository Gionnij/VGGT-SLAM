...
def get_resnet_backbone(cfg, input_shape):
    """
    ...
    deform_on_per_stage = cfg["RESNETS"]["DEFORM_ON_PER_STAGE"]
    deform_modulated    = cfg["RESNETS"]["DEFORM_MODULATED"]
    deform_num_groups   = cfg["RESNETS"]["DEFORM_NUM_GROUPS"]
    # fmt: on
...
def build_resnet_backbone(cfg, input_shape):
    ...
    # fmt: off
    freeze_at           = cfg["backbone"]["freeze"] if "freeze" in cfg["backbone"] else -1
    out_features        = cfg["RESNETS"]["OUT_FEATURES"]
    depth               = cfg["RESNETS"]["DEPTH"]
    num_groups          = cfg["RESNETS"]["NUM_GROUPS"]
    width_per_group     = cfg["RESNETS"]["WIDTH_PER_GROUP"]
    ...
    for idx, stage_idx in enumerate(range(2, 6)):
        ...
        bottleneck_channels *= 2
        stages.append(blocks)
    resnet = ResNet(stem, stages, out_features=out_features, freeze_at=0)
    if "WEIGHTS" in cfg["RESNETS"]:
        print(f"Loading backbone weights from {cfg['RESNETS']['WEIGHTS']}")
        resnet.load_state_dict(
            torch.load(os.path.expanduser(cfg["RESNETS"]["WEIGHTS"]))
        )
    if freeze_at == 0:
        print("---------------frezezing backbone completely----------------")
