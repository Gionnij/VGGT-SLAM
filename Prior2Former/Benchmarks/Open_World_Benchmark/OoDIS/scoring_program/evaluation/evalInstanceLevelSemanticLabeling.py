...
def readPredInfo(predInfoFileName, args):
    ...
    try:
        if not os.path.isfile(predInfoFileName):
            printError(
                "Infofile '{}' for the predictions not found.".format(predInfoFileName)
            )
        with open(predInfoFileName, "r") as f:
            for line in f:
                ...
                if (
                    os.path.commonprefix([filename, args.predictionPath])
                    != args.predictionPath
                ):
                    printError(
...
def printResults(avgDict, args):
    ...
    if args.distanceAvailable:
        ...
        line += (
            getColorEntry(allAp100m, args) + sep + "{:>15.3f}".format(allAp100m) + sep
        )
        line += (
            getColorEntry(allAp5050, args) + sep + "{:>15.3f}".format(allAp5050) + sep
        )
    line += noCol
    print(line)
    print("")
