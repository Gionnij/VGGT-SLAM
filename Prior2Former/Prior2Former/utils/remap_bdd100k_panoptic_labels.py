...
def remap_bdd_panoptic(file,label,target):
    # class colour mapping

    image = Image.open(file)
    img = np.array(image) #w,h,4
    img2=img[:,:,0]
    img2[img2== 33] = 41
    img2[img2 == 37] = 42
    ...
    img[:, :, 0]=img2
    # class group mapping
    savepath=file.replace(label,target)
    os.makedirs(os.path.dirname(savepath), exist_ok=True)
    out_image = Image.fromarray(img)

    # Save the image as a PNG file
    out_image.save(savepath)

def process_bdd(root_path,labels,target):
    file_list=glob.glob(f"{root_path}/{labels}/*/*" )
    for file in file_list:
        remap_bdd_panoptic(file,labels,target)



if __name__ == "__main__":
    root_path="/BDD100k_Anomaly/10k/bdd100k/labels/pan_seg"
    labels="bitmasks"
    target="anom_bitmasks"
    process_bdd(root_path, labels, target)
